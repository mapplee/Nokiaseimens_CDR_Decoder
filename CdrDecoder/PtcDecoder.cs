...
namespace CdrDecoder
{
    internal class PtcDecoder
    {
        public static List<string> Decode(int currentPosition, List<byte> fileData)
        {
            ...
            {
                int length = Nokia.PtcFieldsDetails[f].Length;
                int offset = Nokia.PtcFieldsDetails[f].Offset + currentPosition;
                List<byte> recordBytes = new List<byte>(fileData.Skip(offset).Take(length));
                switch (Nokia.PtcFieldsDetails[f].Operation)
                {
